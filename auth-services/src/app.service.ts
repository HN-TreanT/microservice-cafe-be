...
import {
  PEMISSION_ROLE_REPOSITORY,
  PERMISSION_REPOSITORY,
  ROLE_REPOSITORY,
  USER_REPOSITORY,
} from './constants/repository_enum';
...
export class AppService {
  constructor(
    ...
    @Inject(PEMISSION_ROLE_REPOSITORY)
    private readonly permisionRoleReposity: typeof PermissionRole,
  ...
  async signIn(username: string, password: string) {
    ...
    //get permission
    const permisionRole = await this.permisionRoleReposity.findAll({
      where: { id_role: user.id_role },
    });
    const permissions: string[] = permisionRole.map(
      (item) => item.id_permistion,
    );
  ...
  async refresh(payload: any) {
    try {
      ...
      const verify_refresh_token = await this.jwtService.verifyAsync(
        refresh_token,
        {
          secret: jwtContants.refreshToken_secret,
        },
  ...
  async editRole(dto: RoleDTO) {
    ...
    if (!role)
      throw new NotFoundException({ message: 'not found role', status: false });
  ...
  async editPermisionRole(dto: PermissionRoleDTO) {
    ...
    await this.permisionRoleReposity.destroy({
