...
export class OrderService {
  constructor(
    ...
    @Inject(SHIPMENT_ONLINE_REPOSITORY)
    private readonly shippemntOnlineRepository: typeof ShipmentOnline,
      ...
      rows.map(async (row) => {
        const order_details_custom = await Promise.all(
          row.order_details.map(async (order_detail) => {
            const product = await this.offlineClient
              .send('detail-product', { id: order_detail.id_product })
              .toPromise();
            return {
  ...
  async create(dto: OrderCreateDTO) {
    ...
    try {
      ...
      const totalPrice = dto.order_details.reduce(
        (sum, detail) => sum + detail.quanity * detail.price,
        0,
      );
  ...
  async edit(id: number, dto: OrderCreateDTO) {
    ...
    try {
      ...
      //remove order detail
      await this.orderDetailRepository.destroy({
